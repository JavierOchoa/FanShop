  ...
  async (req, res) => {
    ...
    try {
      const orderOnDb = await orderRepository.findOne({
        where: { id },
        relations: { user: true, products: { product: true }, address: true },
      });
...
internalOrderRouter.get(
  ...
  async (req, res) => {
    const { id } = req.params;
    try {
      const orderOnDb = await orderRepository.findOne({ where: { id } });
      if (!orderOnDb) return res.send(routeResponse(false, "Order Not Found"));
      return res.send(routeResponse(true, "Order found", orderOnDb.status));
    } catch (e) {
